...
export default function MonteSeuAcai() {
  ...
  // --- ESTADO DO PEDIDO (SCHEMA COMPATÍVEL COM O SEU CARRINHO ORIGINAL) ---
  ...
  const [pedido, setPedido] = useState({
    ...
    precoChave: '', 
    tamanho: '',
    acompanhamentos: [], // IDs dos itens grátis para lógica interna
    coberturaId: '',     // ID da cobertura para lógica interna
    colher: '',
  ...
  // --- 3. LÓGICA DE NAVEGAÇÃO E VALIDAÇÃO ---
  const handleNext = () => {
  ...
  // --- 4. INTEGRAÇÃO FINAL: SALVAR NA SACOLA (ESTRUTURA CORRIGIDA PARA O CARRINHO.JSX) ---
const salvarNaSacola = (repetir = 1) => {
    const carrinhoAtual = JSON.parse(localStorage.getItem('carrinho_rodrigues')) || { itens: [], totalGeral: 0 };
    
    // 1. FILTRAGEM PELA ORDEM DO ADMIN (Independente da ordem do clique)
    // Cobertura
    const nomeCobertura = coberturas.find(c => c.id === pedido.coberturaId)?.nome || '';

    // Acompanhamentos Grátis: Filtramos a lista mestre 'gratis' (já ordenada pelo Firebase)
    const nomesGratis = gratis
      .filter(g => pedido.acompanhamentos.includes(g.id))
      .map(g => g.nome);

    // Adicionais Pagos: Filtramos a lista mestre 'adicionais'
    ...
    const itemFormatado = {
      ...
      // Detalhes salvos em blocos organizados para o KDS/Modo Inteligente
      detalhes: {
        baseNome: pedido.baseNome,
        tamanho: pedido.tamanho,
        cobertura_detalhes: nomeCobertura, // Ordem 1
        acompanhamentos_detalhes: nomesGratis, // Ordem 2
        adicionais_detalhes: adicionaisOrdenados, // Ordem 3
        colher: pedido.colher || 'Não solicitada', // Ordem 4
        foto: pedido.foto
      }
    };
              ...
              {bases.map(b => (
                <button 
                  ...
                  onClick={() => {
                    ...
                    // Removemos o avanço automático para você testar como preferir
                  }}
...
{etapa === 2 && (
  <motion.div key="e2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
    ...
    {Object.keys(recipientes)
      /* FILTRO ATÓMICO: Esconde se for explicitamente falso (boleano ou texto) */
      .filter(t => {
        const item = recipientes[t];
        return item.disponivel !== false && item.disponivel !== "false";
      })
      .sort((a,b) => (recipientes[a].ordem ?? 999) - (recipientes[b].ordem ?? 999))
      .map(t => (
        <button key={t} onClick={() => setPedido({...pedido, tamanho: t})}
          className={`w-full p-6 rounded-[2.5rem] flex justify-between items-center border-4 transition-all duration-300 bg-white shadow-xl 
          ${pedido.tamanho === t ? 'border-[#82C91E] scale-[1.02]' : 'border-transparent opacity-90'}`}>
          
          <div className="flex items-center gap-5">
            <div className="w-20 h-20 bg-slate-50 rounded-[1.8rem] p-2 flex items-center justify-center">
              <img src={recipientes[t].imagem_url || 'https://cdn-icons-png.flaticon.com/512/1046/1046751.png'} className="w-full h-full object-contain" alt={t} />
            </div>
          ...
          <div className="text-right">
            <span className="text-xl font-[1000] text-black italic">R$ {Number(recipientes[t][pedido.precoChave] || 0).toFixed(2).replace('.', ',')}</span>
          </div>
        </button>
      ))}
  </motion.div>
          ...
          {etapa === 4 && (
            <motion.div key="e4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                {coberturas.map(c => (
                  <button key={c.id} onClick={() => setPedido({...pedido, coberturaId: c.id})}
          ...
          {etapa === 6 && (
            <motion.div key="e6" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4">
                {colheres.map(c => (
                  <button key={c.id} onClick={() => setPedido({...pedido, colher: c.nome})}
                    ...
                    <div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center p-4">
                       {c.imagem_url ? <img src={c.imagem_url} className="w-full h-full object-contain" alt={c.nome} /> : <Lucide.Utensils size={40} className="text-slate-200"/>}
                    </div>
          ...
          {etapa === 7 && (
            <motion.div key="e7" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
              <div className="bg-white rounded-[4rem] p-8 shadow-2xl border-b-[15px] border-slate-100 relative overflow-hidden">
                ...
                {pedido.logoBase && <img src={pedido.logoBase} className="absolute -top-6 -right-6 w-32 h-32 opacity-10 rotate-12 grayscale" alt="logo agua" />}
              ...
              {/* BOTÕES DE AÇÃO FINAL (SALVA EXATAMENTE COMO O CARRINHO LÊ) */}
     ...
     {/* FOOTER NAVEGAÇÃO (BOTÃO VOLTAR SEMPRE ATIVO) */}
      {etapa < 7 && (
        <footer className="fixed bottom-10 left-1/2 -translate-x-1/2 w-[92%] max-w-[500px] flex gap-4 z-[100]">
          
          {/* Botão Inteligente: Se for Etapa 1, usa o navigate(-1) para sair; senão, retrocede a Etapa */}
          <button 
            onClick={() => etapa === 1 ? navigate(-1) : setEtapa(etapa - 1)} 
            className="w-24 h-24 bg-white rounded-[2.5rem] shadow-2xl flex items-center justify-center text-[#4B0082] border-b-8 border-slate-200 active:scale-90 transition-all"
          >
            {etapa === 1 ? (
              <Lucide.ArrowLeft size={35} strokeWidth={4} />
            ) : (
