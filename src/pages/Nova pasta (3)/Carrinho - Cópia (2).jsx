...
export default function Carrinho() {
  ...
  {item.detalhes?.cobertura_detalhes && (
    <span className="text-[9px] font-black text-white bg-pink-500 px-3 py-1 rounded-lg uppercase italic">
      Calda: {item.detalhes.cobertura_detalhes}
    </span>
  )}
  
  {/* Depois os Grátis (conforme ordem do admin) */}
  {(item.detalhes?.acompanhamentos_detalhes || []).map((acc, i) => (
    <span key={i} className="text-[9px] font-bold text-slate-500 bg-slate-50 border border-slate-100 px-2 py-1 rounded-lg uppercase truncate">
      {acc}
    </span>
  ))}
  
  {/* Por fim os Adicionais */}
  {(item.detalhes?.adicionais_detalhes || []).map((add, i) => (
    <span key={i} className="text-[9px] font-black text-[#4B0082] bg-[#82C91E]/20 border border-[#82C91E]/30 px-2 py-1 rounded-lg uppercase">
      + {add.qtd}x {add.nome}
    </span>
  ))}
</div>
