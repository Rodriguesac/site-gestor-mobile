...
const PainelEntregadoresContent = () => {
    ...
    const salvarEntregador = async (e) => {
        ...
        try {
            ...
            if (editandoId) {
                ...
                await setDoc(doc(db, "entregadores", cpfLimpo), { // Usando CPF como ID igual no app
                    ...payload, 
    ...
    const excluirEntregador = async (id, nome) => {
        if(window.confirm(`ATENÇÃO: Deseja EXCLUIR permanentemente o piloto ${nome}? Esta ação é irreversível.`)) {
            try {
                // Ao deletar o documento, o onSnapshot atualiza a lista em tempo real
    ...
    const salvarNovaSenha = async (e) => {
        ...
        try {
            await updateDoc(doc(db, "entregadores", modalSenha.pilotoId), {
                ...
                solicitouResetSenha: false // Limpa flag se existir
            });
    ...
    // Filtro de Busca (Search Horizontal)
    ...
    const metricas = {
        ...
        totalAPagar: ativos.filter(e => e.saldoLiquido > 0).reduce((acc, e) => acc + e.saldoLiquido, 0), // Loja deve ao piloto
        totalAReceber: ativos.filter(e => e.saldoLiquido < 0).reduce((acc, e) => acc + Math.abs(e.saldoLiquido), 0) // Piloto deve à loja
    };
    ...
    return (
        <div className="flex min-h-screen bg-[#F8FAFC] font-sans selection:bg-[#82C91E]/30">
            <div className="flex-1 overflow-y-auto p-8 md:p-12 relative z-10">
                ...
                {/* ==========================================
                    HEADER E DASHBOARD DE MÉTRICAS
                ========================================== */}
                ...
                {/* PAINEL COMPLETASSO DE MÉTRICAS */}
                ...
                {/* ==========================================
                    LISTA HORIZONTAL (TABELA DE DADOS)
                ========================================== */}
                        ...
                        <AnimatePresence>
                            ...
                            {(tabAtiva === 'ATIVOS' ? ativosFiltrados : pendentes).map(ent => (
                                <motion.div key={ent.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-12 gap-4 p-6 items-center hover:bg-slate-50/50 transition-colors">
                                    ...
                                    {/* 4. FINANCEIRO (Muda se for pendente) */}
            ...
            {/* ========================================================================
                MODAL DE REDEFINIR SENHA (ADMIN)
            ======================================================================== */}
            <AnimatePresence>
                {modalSenha.aberto && (
